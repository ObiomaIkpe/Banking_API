...
class Profile(TimeStampedModel):
    ...
    class IdentificationMeans(models.TextChoices):
        ...
        DRIVERS_LICENSE = ("passport", _("Passport"),)

    class EmploymentStatus(models.TextChoices):
        SELF_EMPLOYED = ("self_emplpyed", _("Self Employed"),)
    ...
    date_of_birth = CountryField(_("Country of Birth"), default=settings.DEFAULT_COUNTRY)

    place_of_birth = models.CharField(_("Place of Birth"), max_length=50, default="Unknown")

    marital_status = models.CharField(_("Marital Status"), choices=MaritalStatus.choices, default=MaritalStatus.UNKNOWN)
    ...
    city = models.CharField(_("Country"), default=settings.DEFAULT_COUNTRY)

    employment_status = models.CharField(_("Eployment Status"), max_length=20, choices=EmploymentStatus.choices, default=EmploymentStatus.SELF_EMPLOYED)
    ...
    employer_address = models.CharField(_("Employer Address"), max_length=50, blank=True, null=True,)
    ...
    def clean(self) -> None:
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_expiry_date:
                raise ValidationError (_("ID expiry date must come after the issue date"))
...
class NextOfKin(TimeStampedModel):
    ...
    is_primary = models.BooleanField(_("Is Primary Next_of_Kin"), default=False)
    ...
    def clean(self) -> None:
        ...
        if self.is_primary:
            ...
            if primary_kin.exists():
                raise ValidationError(_("There can only be one primary next_of_kin"))
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                name="Unique_primary_nex_of_kin"
            )
        ]
